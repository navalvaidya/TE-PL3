...
while True:
	flag=0
	user=raw_input("Enter Floor : ")
	for i in range(0,len(button)):
		g.setup(button[i],g.IN)
	
	if g.input("P8_10")==0 or user=="0":
		flag=1
		destFloor=0
	
	if g.input("P8_8")==0 or user=="1":
		flag=1
		destFloor=1
	
	if g.input("P8_9")==0 or user=="2":
		flag=1
		destFloor=2
    
	if g.input("P8_7")==0 or user=="3":
		flag=1
        	destFloor=3
